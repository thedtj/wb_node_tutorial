...
var mapOptions = {
	center: { lat: 43.255, lng: -79.865 },
	zoom: 10
};

// navigator.geolocation.getCurrentPosition dynamically grabs user location
function loadPlaces(map) {
	var lat = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 43.255;
	var lng = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : -79.865;

	_axios2.default.get('/api/v1/stores/near?lat=' + lat + '&lng=' + lng).then(function (res) {
		var places = res.data;
		if (!places.length) {
			alert('No results at this location.');
			return;
		}
...
function typeAhead(search) {
	...
	searchInput.on('input', function () {
		...
		_axios2.default.get('/api/v1/search?q=' + this.value).then(function (res) {
			...
			searchResults.innerHTML = _dompurify2.default.sanitize('<div className="search__result">No Matching Results for ' + _this.value + '</div>');
		}).catch(function (err) {
			console.error(err);
		});
	});
